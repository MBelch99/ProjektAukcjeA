		...
		<div class="card">
			<div class="card-body">
				<form action="" id="manage-product">
					...
					<h4><b><?php echo !isset($id) ? "Nowy produkt" : "Zarządzanie produktami" ?></b></h4>
					<hr>
					...
					<div class="form-group row">
						<div class="col-md-4">
							<label for="" class="control-label">Kategoria</label>
					...
					<div class="form-group row">
						<div class="col-md-4">
							<label for="" class="control-label">Cena rynkowa</label>
							<input type="number" class="form-control text-right" name="regular_price" value="<?php echo isset($regular_price) ? $regular_price : 0 ?>">
						</div>
						<div class="col-md-4">
							<label for="" class="control-label">Cena początkowa licytacji</label>
							<input type="number" class="form-control text-right" name="start_bid" value="<?php echo isset($start_bid) ? $start_bid : 0 ?>">
						</div>
					</div>
					<div class="form-group row">
						<div class="col-md-4">
							<label for="" class="control-label">Data zakończenia licytacji</label>
							<input type="text" class="form-control datetimepicker" name="bid_end_datetime" value="<?php echo isset($bid_end_datetime) && strtotime($bid_end_datetime) > 0 ? date("d-m-Y H:i",strtotime($bid_end_datetime)) : '' ?>">
						</div>
					</div>
					<div class=" row form-group">
						<div class="col-md-5">
							<label for="" class="control-label">Zdjęcie produktu</label>
					...
					<div class="row">
						<div class="col-md-12">
							<button class="btn btn-sm btn-block btn-primary col-sm-2"> Zapisz</button>
						</div>
					</div>
				</form>
			</div>
...
<script>
	...
	$('#manage-product').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Dane zapisane pomyślnie",'success')
					setTimeout(function(){
  ...
  document.getElementById('status').innerHTML = 'Twoja przeglądarka nie wspiera HTML5';
}
...
function rem_func(_this){
		...
		if($('#drop .imgF').length <= 0){
			$('#drop').append('<span id="dname" class="text-center">Umieść plik tutaj</label></span>')
		}
}
</script>